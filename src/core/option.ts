import { DATE_LIST, SELECT_LIST } from 'global/variable';
...
export const validateOption = (
  ...
  const warnings: TvueOptionWarning[] = [];
  if (!isObject(option)) {
    return [
      {
        path: component,
...
export const warnOption = (
  ...
  warnings.forEach((warning) => {
    ...
    if (typeof console !== 'undefined' && console.warn) {
      console.warn(`[Tvue option warn][${component}] ${warning.path}: ${warning.message}`);
    }
  });
};
