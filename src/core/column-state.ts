...
const STORAGE_PREFIX = 'TVUE_COLUMN_STATE:';
const STATE_KEYS = ['hide', 'fixed', 'filters', 'sortable', 'width'] as const;

export interface TvueColumnStateItem {
  prop: string;
  order: number;
  hide?: boolean;
...
export interface TvueColumnState {
  version: number;
  columns: TvueColumnStateItem[];
}
...
export const createColumnState = (columns: AnyRecord[] | AnyRecord = []): TvueColumnState => {
  return {
    version: 1,
    columns: normalizeColumns(columns)
      .filter((column) => column && column.prop)
      .map((column, index) => {
        const item: TvueColumnStateItem = {
          prop: column.prop,
          order: index,
        };
...
export const applyColumnState = (
  ...
  const stateMap = state.columns.reduce((map, item) => {
    ...
    return map;
  }, {} as Record<string, TvueColumnStateItem>);
...
export const loadColumnState = (key: string, storageType?: string) => {
  ...
  try {
    ...
    return value ? (JSON.parse(value) as TvueColumnState) : null;
  } catch (error) {
    return null;
  }
};
