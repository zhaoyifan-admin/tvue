...
export const isMediaType = (url, type) => {
  ...
  if (
    typeList.audio.test(url) ||
    typeList.audio.test(type) ||
    type == "audio"
  ) {
    return "audio";
  } else if (
    typeList.video.test(url) ||
    typeList.video.test(type) ||
    type == "video"
  ) {
    return "video";
  } else if (
    typeList.img.test(url) ||
    typeList.img.test(type) ||
    type == "img"
  ) {
    return "img";
  }
...
export function findObject(list = [], value, prop = "prop") {
  ...
  for (const ele of list) {
    const column =
      ele.column ||
      (ele.children && CHILDREN_LIST.includes(ele.type)
        ? ele.children.column
        : null);
    if (column) {
