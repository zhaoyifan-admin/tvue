/*! Tvue.js v0.0.19 | (c) 2017-2026 Smallwei | Released under the MIT License. */
import { getDefaultExportFromCjs } from './_commonjsHelpers.mjs';
import { __require as requireAdvancedFormat } from '../node_modules/.pnpm/dayjs@1.11.20/node_modules/dayjs/plugin/advancedFormat.mjs';

var advancedFormatExports = requireAdvancedFormat();
var advancedFormat = /*@__PURE__*/getDefaultExportFromCjs(advancedFormatExports);

export { advancedFormat as default };
